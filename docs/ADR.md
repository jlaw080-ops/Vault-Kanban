...
## ADR-025: 인쇄는 `window.print()` 유지, `printToPDF` 도입하지 않음

- **결정**: 문서 뷰의 인쇄는 렌더러의 `window.print()` + `@media print` CSS로만 처리한다. 메인 프로세스의 `webContents.printToPDF`는 도입하지 않는다.
- **배경**: Windows 인쇄 창의 미리보기 영역에 "이 앱은 인쇄 미리 보기를 지원하지 않습니다"가 표시된다. Chrome의 인쇄 미리보기 UI는 Chrome 브라우저의 기능이지 Electron이 품은 렌더링 엔진의 일부가 아니라서, Electron 앱은 Windows 인쇄 창에 미리보기를 제공하지 못한다.
- **이유**:
  - **출력물 자체는 정상이다.** "Microsoft Print to PDF"로 실측한 결과 지면만 A4로 출력되고, 표 머리행 반복과 `<!-- pagebreak -->` 강제 개시가 모두 의도대로 동작했다. 즉 결함이 아니라 미리보기 영역만 비는 표시상 제약이다.
  - 문서 뷰 화면 자체가 이미 210mm 지면을 흰 배경으로 그대로 보여준다. 사용자에게 필요한 "인쇄물이 어떻게 나오는지"는 이 화면이 충족하므로, 인쇄 창의 미리보기는 중복 기능이다.
  - `printToPDF`를 붙이면 IPC 채널·저장 다이얼로그·파일 쓰기 경로가 새로 생긴다. 표시상 제약 하나를 없애려고 늘리기에는 표면적이 크다.
- **트레이드오프**: 사용자가 인쇄 창에서 결과를 미리 볼 수 없다. PDF로 저장한 뒤 확인해야 한다. 실사용 흐름(문서 뷰에서 확인 → 인쇄)에서는 문제되지 않는다고 판단했다.
- **재검토 조건**: 아래 중 하나가 생기면 `printToPDF` 도입을 다시 검토한다.
  - 파일명·저장 위치를 앱이 정해야 하는 요구 (예: 노트 제목으로 자동 저장)
  - 인쇄 창을 거치지 않는 일괄 PDF 내보내기 요구
  - 여러 노트를 한 PDF로 묶는 요구
