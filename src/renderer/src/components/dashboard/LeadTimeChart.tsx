...
import { computeLeadTime } from '../../lib/metrics'
...
export function LeadTimeChart({ notes }: LeadTimeChartProps): JSX.Element {
  const data = useMemo(() => {
    const times = notes.map((n) => computeLeadTime(n)).filter((v): v is number => v !== undefined)
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={data.rows} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
        ...
        <Tooltip />
        <Bar dataKey="count" fill="#3b82f6" name="건수" radius={[2, 2, 0, 0]} />
        <ReferenceLine
